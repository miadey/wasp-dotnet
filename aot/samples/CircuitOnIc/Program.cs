...
public static class Program
{
    ...
    internal static void Init()
    {
        try
        {
            ...
            // Register the same assets with the HTTP cert tree so they
            // can be served from the query call path (no consensus, ~50 ms
            // instead of ~3 s on local dfx). The IC HTTP gateway accepts
            // the response because it carries an IC-Certificate header
            // generated by IcHttpCertTree.
            EmbeddedStaticFiles.CertifyWaspEmbeddedStaticFiles(typeof(Program).Assembly);
