...
| **S5/S7** | `CircuitHubFacade` → real CircuitHost | mostly done | `CircuitHubFacade.Bind(transport, factory, services)` now calls real `CircuitFactory.CreateCircuitHostAsync(...)` and forwards 6 hub methods (`OnRenderCompleted`, `OnLocationChanged`, `OnLocationChanging`, `EndInvokeJSFromDotNet`, `ReceiveByteArray`, `ReceiveJSDataChunk`) directly to the real `CircuitHost` instance. Outbound `Completion` frames ship via the new `IcCircuitTransport.SendRawFrame`. 7 methods are documented stubs (StartCircuit uses an empty `ComponentDescriptor` list pending ServerComponentDeserializer wiring; BeginInvokeDotNetFromJS needs RemoteJSRuntime cast; UpdateRootComponents/ResumeCircuit/PauseCircuit/SendDotNetStreamToJS need follow-up). |
| **S7** (#60) | Live click-counter demo | scaffold ready | `aot/samples/CircuitOnIc/` — full Razor app with `Counter.razor @rendermode InteractiveServer`, `Program.cs` wiring `IcCircuitTransportRegistry → CircuitHubFacade.Bind(...)` against the real DI container, `circuitonic.did`, `build-and-deploy.sh`, dfx.json entries for both `circuitonic` (backend) and `circuitonic_assets` (frontend). **C# compile succeeds.** Native AOT requires the Linux ILC docker (same as RazorOnIc); end-to-end browser verification awaits the StartCircuit deserializer wiring above. |
