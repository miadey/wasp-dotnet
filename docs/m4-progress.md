...
| **S7** (#60) | Live click-counter demo | AOT builds, dfx install blocked | `aot/samples/CircuitOnIc/` complete. **Full stack AOT-compiled** via `wasp-dotnet-build:latest` docker: 57 MB native wasm32-wasi → 28 MB after icp-publish + wasi-stub → 20 MB after `ic-wasm shrink` + `ic-wasm optimize O4`. **Hits IC's 11 MB code-section limit on install.** Exports verified correct: `canister_query http_request`, `canister_update http_request_update`, `canister_update ws_open/ws_close/ws_message`, `canister_query ws_get_messages`. Further size reduction (additional ILLink substitutions for unused MessagePack resolvers, BlazorPack reflective code paths that BlazorHubDispatcher replaces, ServerComponentSerializer init-only fields) is the remaining gate before mainnet deploy. |
